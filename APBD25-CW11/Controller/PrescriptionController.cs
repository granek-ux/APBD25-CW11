...
namespace APBD25_CW11.Controller
{
    ...
    public class PrescriptionController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetPrescriptions(int patientId,CancellationToken cancellationToken)
        {
            try
            {
                var patient = await _dbService.getPatient(patientId, cancellationToken);
                return Ok(patient);
            }
            catch (NotFoundException ex)
            {
